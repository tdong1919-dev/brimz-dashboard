...
import { useDemographics, useFanSegments, useEngagementBreakdown } from '../api/queries'
...
// Icon/color per KPI label; the value comes from the live engagement breakdown.
const KPI_META = [
  { label: 'Poll Participation', icon: '📊', color: '#14b8a6' },
  { label: 'Challenges Joined', icon: '🏆', color: '#f59e0b' },
  { label: 'Rewards Claimed', icon: '🎁', color: '#a855f7' },
  { label: 'Social Shares', icon: '📱', color: '#14b8a6' },
]

// demo-static: no per-age-group engagement score endpoint in MVP (age brackets are live)
const AGE_ENGAGEMENT: Record<string, number> = {
  '18–24': 82, '25–34': 91, '35–44': 87, '45–54': 74, '55+': 61,
}

// demo-static: no sentiment endpoint in MVP
const sentiment = [
      ...
      <QueryBoundary query={engagementQ} compact>
        {(data) => (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {KPI_META.map((k) => {
              const stat = data.stats.find((s) => s.label === k.label)
              return (
                <div key={k.label} className="bg-[#141824] border border-[#2a2f3e] rounded-xl p-4">
                  <div className="text-2xl mb-2">{k.icon}</div>
                  <div className="text-xl font-bold" style={{ color: k.color }}>{stat?.value ?? '—'}</div>
            ...
            {(demographics) => (
              <>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={demographics.ageGroups.map((a) => ({ ...a, engagement: AGE_ENGAGEMENT[a.group] ?? 0 }))}>
            ...
            {(demographics) => (
              <div className="flex items-center gap-6 py-4">
                <div className="text-center">
                  <div className="text-5xl font-black text-[#14b8a6]">{demographics.returning}%</div>
                  <div className="text-sm text-[#64748b] mt-2">Returning Fans</div>
                </div>
                <div className="flex-1 space-y-3">
                  {/* demo-static: spend-vs-first-timer & avg-events metrics not exposed by the API */}
